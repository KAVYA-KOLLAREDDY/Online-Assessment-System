...
//   styleUrls: ['./exam-questions.component.css'],
// })
// export class ExamQuestionsComponent implements OnInit, OnDestroy, AfterViewInit {
//   @ViewChild('stepper') stepper!: ElementRef<HTMLDivElement>;
...
//   showStepperModal(): void {
//     this.stepperModalVisible = true;
//     setTimeout(() => this.scrollToCurrentQuestion(), 0);
//   }

//   closeStepperModal(): void {
//     this.stepperModalVisible = false;
//   }

//   private scrollToCurrentQuestion(): void {
//     if (!this.stepper || !this.stepper.nativeElement) return;

//     const stepperElement = this.stepper.nativeElement;
//     const stepWrappers = stepperElement.querySelectorAll('.step-wrapper');
//     const currentStepWrapper = stepWrappers[this.currentQuestionIndex] as HTMLElement;

//     if (currentStepWrapper) {
//       const stepWidth = currentStepWrapper.offsetWidth;
//       const stepperWidth = stepperElement.offsetWidth;
//       const scrollPosition = currentStepWrapper.offsetLeft - (stepperWidth / 1) + (stepWidth / 1);
...
//     const examId = Number(examIdParam);
//     if (!examIdParam || isNaN(examId)) {
//       this.showModal('Error', 'Invalid exam ID.', () => this.router.navigate(['/']));
//       return;
//     }
...
//         console.log('Exam Response:', exam);
//         this.attempts = exam.attempts;
//         this.examService.createAttempt(examId, exam.attempts).subscribe({
//           next: () => {
//             localStorage.setItem("attemptStatus", "ONGOING");
//           }
//         });
...
//       error: (err) => {
//         console.error('Get Exam Error:', err);
//         this.showModal('Error', 'Failed to load exam details.', () => this.router.navigate(['/']));
//       },
//     });
...
//         console.log('Questions Response:', response);
//         if (!response.questions || response.questions.length === 0) {
//           this.showModal('Error', 'No questions available for this exam.', () => this.router.navigate(['/']));
//           return;
//         }
//         this.questions = response.questions;
//         console.log("Questions", this.questions);
//         this.subtopicQuestionCounts = response.subtopicQuestionCounts;
//         console.log(this.subtopicQuestionCounts);
//       },
//       error: (err) => {
//         console.error('Get Questions Error:', err);
//         this.showModal('Error', 'Failed to load questions: ' + (err.error || 'Unknown error'), () => this.router.navigate(['/']));
//       },
//     });
...
//     const currentQuestion = this.questions[this.currentQuestionIndex];
//     if (!(currentQuestion.questionId in this.selectedAnswers)) {
//       this.showModal('No Answer Selected', 'Please select an answer before submitting.', () => {});
//       return;
//     }

//     const selected = this.selectedAnswers[currentQuestion.questionId];
//     const selectedIds = Array.isArray(selected) ? new Set(selected) : new Set([selected]);
//     const correctIds = new Set(currentQuestion.options.filter(o => o.isCorrect).map(o => o.optionId));

//     if (currentQuestion.questionType === 'MSQ') {
//       this.isCorrect = selectedIds.size === correctIds.size && [...selectedIds].every(id => correctIds.has(id));
//     } else {
//       this.isCorrect = selectedIds.size === 1 && correctIds.has([...selectedIds][0]);
//     }

//     this.questionResults[currentQuestion.questionId] = this.isCorrect;

//     this.isQuestionSubmitted = true;
//     this.showStepperModal();
//   }
...
//     } else {
//       console.log("UNSUCCESSFUL EXAM STATUS!");
//       let responses = Object.entries(this.selectedAnswers).map(
//         ([qId, sel]) => {
//           console.log(`Question id : ${qId}, Selected Opt : ${sel}`);
//           return {
//             question: +qId,
//             selectedOption: Array.isArray(sel) ? sel : [sel],
//           };
...
//       console.log(`Current Question IDX : ${this.currentQuestionIndex}`);
//       if (this.currentQuestionIndex < responses.length) {
//         this.currentQuestionIndex++;
//       }

//       while (this.currentQuestionIndex < this.questions.length) {
//         responses.push({
//           question: this.questions[this.currentQuestionIndex++].questionId,
//           selectedOption: [],
//         });
//         console.log("TIRGUDINGS");
//         console.log("QUESTIONS : ", this.questions);
...
//     this.showModal(
//       'Error',
//       'Unable to submit exam: Invalid user or exam ID.',
...
//   console.log('Submitting payload:', JSON.stringify(payload)); // Debug payload

//   this.examService.submitResponses(payload).subscribe({
//     next: (res) => {
//       console.log('Submission response:', res); // Debug response
//       this.examStatusLoader = false;
//       this.showModal(
//         'Violation Detected !!',
//         `${reason}. Your exam has been submitted.`,
//         () => {
//           if (res.resultId !== undefined) {
//             console.log('Navigating to result page with resultId:', res.resultId);
//             this.router.navigate([`/student/results/${res.resultId}`], {
...
//             this.showModal(
//               'Error',
//               'No result ID returned. Redirecting to home page.',
//               () => this.router.navigate(['/'])
//             );
//           }
//         }
...
//     },
//     complete: () => {
//       console.log('Submission request completed'); // Debug completion
//     }
//   });
...
//   get correctOptions(): { optionId: number; optionText: string }[] {
...
// export class ExamQuestionsComponent implements OnInit, OnDestroy, AfterViewInit {
//   @ViewChild('stepper') stepper!: ElementRef<HTMLDivElement>;
//   @ViewChild('codeBlock') codeBlock!: ElementRef<HTMLDivElement>;
...
//   examStatusLoader: boolean = false;
//   questionResults: Record<number, boolean | null> = {};
//   subtopicQuestionCounts: { [subtopic: string]: number } = {}; // New property
...
//   // Modal properties (existing)
//   modalVisible = false;
//   modalTitle = '';
//   modalMessage = '';
//   modalCallback: () => void = () => {};

//   // Stepper modal properties
//   stepperModalVisible = false;
...
//   showStepperModal(): void {
//     this.stepperModalVisible = true;
//     setTimeout(() => this.scrollToCurrentQuestion(), 0); // Ensure DOM is updated before scrolling
//   }
...
//   ngAfterViewInit(): void {
//     // Initial scroll if the modal is already visible (unlikely but for safety)
//     if (this.stepperModalVisible) {
//       this.scrollToCurrentQuestion();
//     }
//     // hljs.highlightAll();
...
//           html: this.codeBlock.nativeElement.innerHTML,
//           text: this.codeBlock.nativeElement.innerHTML
//         })
//       }
//     })
//   }
...
//     .replace(/{/g, '{\n')
//     .replace(/}/g, '\n}')
//     // Add newline after semicolon if not inside parentheses (e.g. for loops)
//     .replace(/;(?![^\(\[]*[\)\]])/g, ';\n')
//     // Add newline after colon in Python-style code (basic support)
//     .replace(/:(?![^\(\[]*[\)\]])/g, ':\n')
//     // Split into lines
//     .split('\n')
//     .map(line => line.trim())
...
//   };
// }
//   private submitExam(successfullExamStatus: boolean): void {
//     if (this.examSubmitted) return;
//     this.examSubmitted = true;
...
//     this.showModal(
//       'Violation Detected !!',
//       `${reason} . Your exam will be Submitted!!`,
//       () => this.submitExam(false)
//     );
//     this.stopTimer();
//   }
...
//     this.examService.getExamById(examId).subscribe(
//       handleResposne(this.loggingService, (exam) => {
//         this.durationMinutes = exam.duration ?? 0;
//         this.startTimer(this.durationMinutes * 60);
//       })
//     );
...
//     this.examService.getQuestions(examId).subscribe(
//       handleResposne(this.loggingService, (qs) => {
//         this.questions = qs;
//       })
//     );
...
//   if (!(currentQuestion.questionId in this.selectedAnswers)) {
//     // Mark it as unanswered with empty selectedOption (0 marks)
//     this.selectedAnswers[currentQuestion.questionId] = [];
//   }
...
//     const m = text.match(/^(.*)'(.*)'/);
//     return m
//       ? { question: m[1].trim(), code: m[2].trim() }
//       : { question: text, code: null };
//   }
...
//     if (this.examSubmitted) return;
//     alert(
//       `Illegal activity detected (${reason}), so your exam is getting submitted.`
//     );
//     this.stopTimer();
//     this.submitExam(false);
//   }
...
//       if (!this.examSubmitted) {
//         history.pushState(null, '', location.href);
//         this.autoSubmitDueToViolation('Back button');
//       }
//     };
//   }
...
//   // 🚫 Prevent Ctrl/Meta+A/C/V/R and Escape
//   @HostListener('document:keydown', ['$event'])
//   handleKeydown(event: KeyboardEvent): void {
//     const key = event.key.toLowerCase();
...
//     if (key === 'f5' || (event.ctrlKey && key === 'r')) {
//       event.preventDefault();
//       this.autoSubmitDueToViolation('Page reload attempt');
//     } else if (key === 'escape') {
//       event.preventDefault();
//       this.autoSubmitDueToViolation('Escape key pressed');
//     } else if (event.ctrlKey || event.metaKey) {
...
//   // 🚫 Blur (Alt+Tab or Windows key)
//   @HostListener('window:blur')
//   onWindowBlur(): void {
//     if (!this.examSubmitted) {
//       setTimeout(() => {
//         if (!document.hasFocus()) {
//           this.autoSubmitDueToViolation('Tab switch or Windows key');
//         }
//       }, 300);
//     }
//   }
